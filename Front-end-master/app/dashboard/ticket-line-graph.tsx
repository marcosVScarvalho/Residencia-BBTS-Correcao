...
export const TicketLineGraph = () => {
  ...
  const generateGraphData = useCallback((dayAmount: number): number[] => {
    if (!ticketData || ticketData.length === 0) {
      return Array(dayAmount + 1).fill(0); 
    }
  ...
  }, [ticketData, getLastDays]);

  useEffect(() => {
    if (dateRange?.from && dateRange?.to) {
      setDayAmount(daysBetweenDates(dateRange.from, dateRange.to));
    }
  }, [dateRange, daysBetweenDates]);

  if (!ticketData || !Array.isArray(ticketData)) {
    return <p className="text-gray-500 italic">Carregando gráfico de tickets...</p>;
  }
